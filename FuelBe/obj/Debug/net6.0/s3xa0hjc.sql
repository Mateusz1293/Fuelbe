...
CREATE TABLE [dbo].[refueling] (
    ...
    CONSTRAINT [PK_refueling] PRIMARY KEY ([id]),
    CONSTRAINT [FK_refueling_user_user_id] FOREIGN KEY ([user_id]) REFERENCES [dbo].[user] ([id]) ON DELETE CASCADE,
    CONSTRAINT [FK_refueling_vehicle_vehicle_id] FOREIGN KEY ([vehicle_id]) REFERENCES [dbo].[vehicle] ([id]) ON DELETE CASCADE
);
GO

CREATE INDEX [IX_refueling_user_id] ON [dbo].[refueling] ([user_id]);
GO

CREATE INDEX [IX_refueling_vehicle_id] ON [dbo].[refueling] ([vehicle_id]);
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230626121732_add-refueling', N'6.0.16');
GO
